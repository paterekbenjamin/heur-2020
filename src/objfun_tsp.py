...
class TSPGrid(ObjFun):
    ...
    def __init__(self, par_a, par_b, norm=2):
        ...
        :param p: probability of using the closest city in each step of tour in generating initial population
        """
    ...
    def generate_point_2(self, p):
        """
        New point generator
        """
        nvc = np.arange(1, self.n)  # not visited cities
        tour = []
        vc = [0]  # visited cities
        ac = 0  # actual city
        for i in np.arange(self.n-1):
            if np.random.uniform() > p:
                iac = np.random.randint(self.a[i], self.b[i] + 1)  # index of actual city
                ac = nvc[iac]
                nvc = np.delete(nvc, np.argwhere(nvc==ac)[0], axis=0)  # delete actual city from not visited cities
                tour.append(iac)
                vc.append(ac)
            else:
                distances = self.dist[ac, :]
                distances = np.argsort(distances)
                for city in vc:
                    distances = np.delete(distances, np.argwhere(distances==city)[0], axis=0)
                ac = distances[0]
                iac = (np.argwhere(nvc == ac))[0][0]
                nvc = np.delete(nvc, np.argwhere(nvc==ac)[0], axis=0)
                tour.append(iac)
                vc.append(ac)
