...
class MatchCrossoverMax(Crossover):
    """
    Crossover based on match of parents (details explained in notebook)
    """
    ...
    def crossover(self, x, y):
        ...
        z = np.array([x[i] if x[i] == y[i] else np.random.randint(np.max([x[i], y[i]]) + 1) for i in np.arange(x.size)],
                     dtype=x.dtype)
        
        return z

    
class MatchCrossoverRnd(Crossover):
    """
    Crossover based on match of parents (details explained in notebook)
    """
    ...
    def crossover(self, x, y):
        ...
        z = np.array([x[i] if x[i] == y[i] else np.random.randint(x.size - i) for i in np.arange(x.size)],
                     dtype=x.dtype)
        
        return z
    

class MisMatchCrossoverMax(Crossover):
    """
    Crossover based on mismatch of parents (details explained in notebook)
    """
    ...
    def crossover(self, x, y):
        ...
        z = np.array([x[i] if not x[i] == y[i] else np.random.randint(y[i] + 1) for i in np.arange(x.size)],
                     dtype=x.dtype)
        
        return z

    
class MisMatchCrossoverRnd(Crossover):
    """
    Crossover based on mismatch of parents (details explained in notebook)
    """
    ...
    def crossover(self, x, y):
        ...
        z = np.array([x[i] if not x[i] == y[i] else np.random.randint(x.size - i) for i in np.arange(x.size)],
                     dtype=x.dtype)
...
class GeneticOptimization(Heuristic):
    ...
    def __init__(self, of, maxeval, N, M, Tsel1, Tsel2, mutation, crossover, p):
    ...
    def search(self):
        try:
            ...
            for i in np.arange(self.N):
                x = self.of.generate_point_2(p=self.p)
                pop_X[i, :] = x
